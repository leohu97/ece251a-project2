...
//-----------------------------------------------------------------------------
// Generic line buffer
SI_SEGMENT_VARIABLE(Line[DISP_BUF_SIZE]=0, uint8_t, RENDER_LINE_SEG);
...
void DrawScreenText1(SI_VARIABLE_SEGMENT_POINTER(str, char, RENDER_STR_SEG),uint8_t x, uint8_t y)
{
    uint8_t i;
    uint8_t len = RENDER_GetStrSize(str);
